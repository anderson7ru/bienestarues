...
class Cie10Autocomplete(autocomplete.Select2QuerySetView):
    def get_queryset(self):
        ...
        qs = db29179_cie10.objects.all()
        if self.q:
            #qs = qs.filter(dec10__istartswith=self.q)
            qs = qs.filter(dec10__icontains=self.q)
        return qs
...
def consulta_create(request, pk):
    ...
    if request.method == 'POST':
        ...
        consultaform = ConsultaMForm(initial={'cod_expediente': pk, 'cod_doctor': docpk})
...
def ordenlab_create(request, pk, cpk):
    ...
    if request.method == 'POST':
        print request.POST.get('cod_consulta')
        
        ordenlabform = OrdenLabMForm(request.POST)
        if ordenlabform.is_valid():
            ordenlabform.save()
            return redirect('ordenlab-crear', pk=pk, capk=cpk)
    else:
        ordenlabform = OrdenLabMForm(initial={'cod_expediente': pk, 'cod_doctor': docpk, 'cod_consulta': cpk})
    return render(request, 'generalapp/ordenlab-create.html', {'pacientepk': pacientepk, 'ordenlabform': ordenlabform})
